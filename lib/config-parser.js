...
function processWidgetData(data, widgetConfig, session, extManager) {
    ...
    if (data["@"]) {
        ...
        if (data["@"].id && RegExp('://').test(data["@"].id)) {
            // Check for a W3C-conformant IRI ID and convert it to reverse-dotted notation
            // 1. Drop the scheme, colon, and slashes.
            widgetConfig.id = data["@"].id.substr(data["@"].id.indexOf("://") + 3);
            ...
            widgetConfig.id = widgetConfig.id.replace(/^www\./, "");
            // 3) Reverse the domain name
            widgetConfig.id = widgetConfig.id.substr(0, widgetConfig.id.indexOf("/")).split(".").reverse().toString().replace(/,/g, ".").concat(widgetConfig.id.substr(widgetConfig.id.indexOf("/")));
            // 4) Drop any query or fragment identifiers (? and/or #)
            widgetConfig.id = widgetConfig.id.replace(/(\?|#).*$/, "");
            // 5) Drop any file extension at the end of the IRI (a dot followed by 2-5 characters should be sufficient)
            widgetConfig.id = widgetConfig.id.replace(/\..{2,5}$/, "");
            // 6) Convert any remaining slashes into dots
            widgetConfig.id = widgetConfig.id.replace(/\//g, ".");
        } else {
            // Otherwise use their stated id.
